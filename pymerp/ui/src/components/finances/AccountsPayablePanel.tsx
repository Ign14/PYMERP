...
import { getAccountsPayable, type AccountPayable, type PaginatedResponse } from "../../services/client";

type StatusFilter = "ALL" | "PENDING" | "OVERDUE" | "DUE_SOON";

export default function AccountsPayablePanel() {
  const [data, setData] = useState<PaginatedResponse<AccountPayable> | null>(null);
    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl shadow-lg">
      <div className="p-5 border-b border-neutral-800">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-neutral-100">Cuentas por Pagar</h2>
          <div className="flex gap-2">
            {(["ALL", "PENDING", "DUE_SOON", "OVERDUE"] as StatusFilter[]).map((status) => (
              ...
              >
                {status === "ALL" ? "Todas" : status === "PENDING" ? "Pendientes" : status === "DUE_SOON" ? "Por vencer" : "Vencidas"}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-neutral-950">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-neutral-400 uppercase">Proveedor</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-neutral-400 uppercase">Documento</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-neutral-400 uppercase">Monto</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-neutral-400 uppercase">Saldo</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-neutral-400 uppercase">Emisión</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-neutral-400 uppercase">Vencimiento</th>
              <th className="px-4 py-3 text-center text-xs font-medium text-neutral-400 uppercase">Días</th>
              <th className="px-4 py-3 text-center text-xs font-medium text-neutral-400 uppercase">Estado</th>
            </tr>
          </thead>
            ...
            {loading ? (
              ...
              data.content.map((item) => (
                <tr key={item.id} className="hover:bg-neutral-800">
                  <td className="px-4 py-3 text-sm text-neutral-100">{item.supplierName}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-neutral-400">{formatDate(item.issuedAt)}</td>
      ...
      {data && data.totalPages > 1 && (
        <div className="px-5 py-4 border-t border-neutral-800 flex items-center justify-between">
          <div className="text-sm text-neutral-400">
            Mostrando {page * pageSize + 1} - {Math.min((page + 1) * pageSize, data.totalElements)} de {data.totalElements}
          </div>
          <div className="flex gap-2">
            <button
