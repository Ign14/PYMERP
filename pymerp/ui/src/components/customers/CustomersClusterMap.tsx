...
export default function CustomersClusterMap({
  ...
  const mapCenter = useMemo(() => {
    ...
    const avgLat =
      customersWithGPS.reduce((sum, c) => sum + parseFloat(`${c.lat}`), 0) / customersWithGPS.length
    const avgLng =
      customersWithGPS.reduce((sum, c) => sum + parseFloat(`${c.lng}`), 0) / customersWithGPS.length
    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
      ...
      {customersWithGPS.length === 0 ? (
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg p-8 text-center">
          <p className="text-neutral-400">No hay clientes con coordenadas GPS disponibles</p>
        </div>
      ) : (
        <>
          <div
            className="bg-neutral-800 border border-neutral-700 rounded-lg overflow-hidden mb-4"
            style={{ height: '500px' }}
          >
            <iframe
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs font-mono text-neutral-500">
                    📍 {parseFloat(`${customer.lat}`).toFixed(4)},{' '}
                    {parseFloat(`${customer.lng}`).toFixed(4)}
                  </p>
                  {stats && (
