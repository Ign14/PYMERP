  ...
  const columns = useMemo<ColumnDef<Customer>[]>(
    () => [
            ...
            {customerStats[info.row.original.id] && (
              <span
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium border ${
                  getHealthStatus(customerStats[info.row.original.id]?.lastSaleDate || undefined)
                    .color
                }`}
              >
                <span>
      ...
      },
      {
        ...
        header: 'RUT',
        cell: info => (
          <span className="mono text-sm text-neutral-300">
      ...
      },
      {
        ...
        header: 'Email',
        cell: info => (
          <span className="mono text-sm text-neutral-400">
            {(info.getValue() as string) || '-'}
          </span>
        ),
      },
      ...
      },
      {
        ...
        header: 'Segmento',
        cell: info => (
          <span className="text-sm text-neutral-300">{(info.getValue() as string) || '-'}</span>
        ),
      },
      {
        id: 'totalSales',
        ...
        cell: info => (
          <span className="text-neutral-100 font-semibold">{info.getValue() as number}</span>
        ),
      },
      {
        id: 'totalRevenue',
