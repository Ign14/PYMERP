  ...
  return (
    <Modal open={open} onClose={onClose} title="Registrar Compra">
      <form className="form-grid" onSubmit={handleSubmit}>
        ...
        <div className="item-builder" style={{ 
          gridColumn: "1 / -1",
          display: "grid",
            ...
            <table className="table" style={{ minWidth: "600px" }}>
              ...
              <tbody>
                {items.map((item, idx) => {
                  ...
                  const itemName = product?.name ?? service?.name ?? (item.productId || item.serviceId);
                  ...
                  return (
                    <tr key={`item-${idx}`}>
                      ...
                      <td className="mono" style={{ textAlign: "right" }}>${item.unitCost.toFixed(2)}</td>
                      <td className="mono" style={{ textAlign: "right", fontWeight: "600" }}>
                        ${subtotal.toFixed(2)}
                      </td>
                      <td className="mono" style={{ fontSize: "0.85rem", color: "#666" }}>
                        {item.productId && item.expDate && `Venc: ${new Date(item.expDate).toLocaleDateString("es-CL")}`}
                        {item.productId && item.locationId && locationsQuery.data?.find(l => l.id === item.locationId)?.code}
                        {item.serviceId && "-"}
                      </td>
                      <td>
                        <button 
