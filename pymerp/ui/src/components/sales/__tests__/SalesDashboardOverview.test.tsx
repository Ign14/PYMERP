...
describe('SalesDashboardOverview', () => {
  it('muestra los totales del conjunto de datos de ejemplo', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('sales-total-14d')).toHaveTextContent(
        currencyFormatter.format(total)
      )
    )
    expect(screen.getByTestId('sales-daily-average')).toHaveTextContent(
      currencyFormatter.format(average)
    )
    expect(screen.getAllByTestId('sales-trend-point')).toHaveLength(points.length)
    expect(
      screen.queryByText(/Sin ventas registradas en los últimos días/i)
  ...
  it('actualiza los indicadores al acotar el rango', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('sales-total-14d')).toHaveTextContent(
        currencyFormatter.format(lastPoint.total)
      )
    )
    expect(screen.getByTestId('sales-daily-average')).toHaveTextContent(
      currencyFormatter.format(lastPoint.total)
