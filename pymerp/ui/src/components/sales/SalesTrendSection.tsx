    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl shadow-lg p-5 mb-6">
      <header className="flex flex-wrap gap-4 justify-between items-start mb-4">
        <div>
          ...
          <p className="text-sm text-neutral-400">
            Promedio diario: {formatCurrency(averageDailySales)} | 
            Total período: {formatCurrency(salesData.reduce((sum, point) => sum + point.total, 0))}
          </p>
        </div>
      ...
      <div className="bg-neutral-800 border border-neutral-700 rounded-lg p-4">
        {salesQuery.isLoading ? (
          <p className="text-neutral-400" role="status">Cargando datos de ventas...</p>
        ) : salesQuery.isError ? (
