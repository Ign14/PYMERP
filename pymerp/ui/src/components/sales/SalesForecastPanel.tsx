...
function generateForecast(days: number): ForecastPoint[] {
  ...
  const trend =
    (historical[historical.length - 1].actual - historical[0].actual) / historical.length
...
export default function SalesForecastPanel({ days = 7 }: SalesForecastPanelProps) {
  ...
  const historicalAvg =
    historicalData.reduce((sum, h) => sum + (h.actual || 0), 0) / historicalData.length
    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl shadow-lg p-5 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg p-4">
          ...
          <p
            className={`text-2xl font-bold ${growthRate >= 0 ? 'text-green-400' : 'text-red-400'}`}
          >
            {growthRate >= 0 ? '+' : ''}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <ul className="text-xs text-blue-300 space-y-1">
              ...
              <li>
                • La confianza disminuye con días más lejanos (alta: 1-3 días, media: 4-7 días,
                baja: 8+ días)
              </li>
              <li>
                • Factores externos (promociones, temporadas, eventos) pueden afectar la precisión
              </li>
            </ul>
          </div>
        </div>
