...
vi.mock('../../services/client', async () => {
  ...
  return {
    ...
    createCustomer: (...args: Parameters<typeof actual.createCustomer>) =>
      createCustomerMock(...args),
  }
})
...
describe('CustomerSelect', () => {
  ...
  it('valida los campos mínimos del formulario de creación', async () => {
    ...
    expect(
      await within(dialog).findByText(/El nombre debe tener al menos 3 caracteres/i)
    ).toBeInTheDocument()
