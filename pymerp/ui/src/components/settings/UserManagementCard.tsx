...
export default function UserManagementCard() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (created) => {
      ...
      setFeedback(`Usuario ${created.email} creado. Comparte el enlace de acceso con la nueva clave.`);
    },
  });
    ...
    <div className="card">
      <div className="card-header">
        ...
        <div className="card-actions">
          <button className="btn-link" onClick={() => usersQuery.refetch()} disabled={usersQuery.isFetching}>
            {usersQuery.isFetching ? "Actualizando..." : "Actualizar"}
          </button>
          <button className="btn-link" onClick={() => setCreateOpen(true)} disabled={createMutation.isPending}>
            Nuevo usuario
          </button>
        </div>
      </div>
                ...
                {users.map((user) => {
                  ...
                  return (
                    <tr key={user.id} style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }}>
                      ...
                      <td style={{ padding: "8px", fontFamily: "monospace", fontSize: "0.9rem" }}>{user.email}</td>
                      <td style={{ padding: "8px" }}>{roleText}</td>
                      <td style={{ padding: "8px" }}>{user.status === "active" ? "Activo" : "Suspendido"}</td>
      ...
      <UserFormDialog
        ...
        onSubmit={(values) => (editingUser ? handleUpdateSubmit(values, editingUser.id) : Promise.resolve())}
...
function UserFormDialog({ mode, open, onClose, initialUser, onSubmit, isSubmitting }: UserFormDialogProps) {
...
function PasswordResetDialog({ open, onClose, user, onSubmit, isSubmitting }: PasswordResetDialogProps) {
  ...
  return (
    ...
    >
      <form className="stack gap-sm" onSubmit={handleSubmit} noValidate>
        <p>
          Se generara una nueva contrasena para <strong>{user.email}</strong>. Comparte este valor y pide que la cambien al ingresar.
        </p>
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          <input
            className="input"
