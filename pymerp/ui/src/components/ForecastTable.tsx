...
export default function ForecastTable() {
  ...
  if (error) {
    ...
    return (
      <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-6">
        <p className="text-red-400 font-semibold mb-2">⚠️ Error al cargar pronósticos</p>
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'understocked':
        return (
          <span className="px-2 py-1 text-xs font-semibold rounded-full bg-red-900/40 border border-red-800 text-red-400">
            Stock Bajo
          </span>
        )
      case 'overstocked':
        return (
          <span className="px-2 py-1 text-xs font-semibold rounded-full bg-orange-900/40 border border-orange-800 text-orange-400">
            Sobre-stock
          </span>
        )
      case 'optimal':
        return (
          <span className="px-2 py-1 text-xs font-semibold rounded-full bg-green-900/40 border border-green-800 text-green-400">
            Óptimo
          </span>
        )
      default:
        return (
          <span className="px-2 py-1 text-xs font-semibold rounded-full bg-neutral-800 border border-neutral-700 text-neutral-400">
            {status}
          </span>
        )
    }
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-neutral-800">
              ...
              <th className="text-right p-3 text-sm font-semibold text-neutral-400">
                Stock Actual
              </th>
              <th className="text-right p-3 text-sm font-semibold text-neutral-400">
                Demanda (30d)
                ...
                <td className="p-3 text-right">
                  {forecast.recommendedOrderQty > 0 ? (
                    <span className="text-yellow-400 font-semibold">
                      {forecast.recommendedOrderQty.toFixed(0)}
                    </span>
                  ) : (
