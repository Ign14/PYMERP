...
export default function CustomerSelect({
  ...
  const customers = useMemo(
    () => mergeCustomerPages(customersQuery.data?.pages ?? []),
    [customersQuery.data]
  )
  ...
  return (
    <div className="customer-select" ref={containerRef}>
      <label className="customer-select__label" htmlFor={`${listboxId}-input`}>
        {label}
        {required ? (
          <span className="customer-select__required" aria-hidden="true">
            *
          </span>
        ) : null}
      </label>
      <div className="customer-select__field">
        ...
        >
          <input
            ...
            aria-activedescendant={
              highlightedIndex >= 0 && customers[highlightedIndex]
        ...
        {error ? (
          <p
            id={`${listboxId}-error`}
            className="customer-select__feedback customer-select__feedback--error"
            role="alert"
          >
            {error}
          </p>
      ...
      {createOpen ? (
        <div
          className="customer-create-overlay"
          role="dialog"
          aria-modal="true"
