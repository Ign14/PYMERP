...
export function PurchasesSupplierStatsPanel({ startDate, endDate }: Props) {
  ...
  const supplierStats = useMemo<SupplierStats[]>(() => {
    ...
    const statsArray = Array.from(statsMap.values()).map(stat => ({
      ...
      daysSinceLastOrder: stat.lastOrderDate 
        ? Math.floor((now.getTime() - new Date(stat.lastOrderDate).getTime()) / (1000 * 60 * 60 * 24))
        : 999,
    }));
  ...
  if (!supplierStats.length) {
    return (
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl shadow-lg p-5">
        <h3 className="text-neutral-100">Top 10 Proveedores</h3>
        <p className="muted text-neutral-400">No hay datos de proveedores en el período seleccionado</p>
      </div>
    );
  }

  const chartData = supplierStats.map(stat => ({
    name: stat.supplierName.length > 20 ? stat.supplierName.substring(0, 17) + "..." : stat.supplierName,
          ...
          <tbody>
            {supplierStats.map((stat) => (
              <tr key={stat.supplierId}>
                ...
                <td>
                  {stat.lastOrderDate 
                    ? new Date(stat.lastOrderDate).toLocaleDateString()
                    : "N/A"
                  }
                </td>
                <td>
      ...
      <div style={{ marginTop: "1rem" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <p style={{ fontSize: "1.5rem", fontWeight: "bold" }} className="text-neutral-100">{supplierStats.length}</p>
          </div>
          <div>
