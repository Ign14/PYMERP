...
export default function PurchaseABCChart() {
  ...
  if (!classifications || classifications.length === 0) {
    return (
      <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-white mb-4">📊 Distribución ABC de Proveedores</h3>
        <p className="text-neutral-400 text-center py-8">No hay datos suficientes para análisis ABC</p>
      </div>
    );
  }
  ...
  const chartData = [
    { name: "Clase A (Críticos)", proveedores: summary.A.count, gasto: summary.A.totalSpent, porcentaje: summary.A.percentage },
    { name: "Clase B (Importantes)", proveedores: summary.B.count, gasto: summary.B.totalSpent, porcentaje: summary.B.percentage },
    { name: "Clase C (Ocasionales)", proveedores: summary.C.count, gasto: summary.C.totalSpent, porcentaje: summary.C.percentage },
  ];
  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-6">
      <h3 className="text-lg font-semibold text-white mb-6">📊 Distribución ABC de Proveedores (Pareto 80-15-5)</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="bg-gradient-to-br from-red-900/30 to-red-800/20 border border-red-800 rounded-lg p-4">
          ...
          <div className="text-sm text-red-200 mt-1">
            {formatCurrency(summary.A.totalSpent)}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-900/30 to-amber-800/20 border border-amber-800 rounded-lg p-4">
          ...
          <div className="text-sm text-amber-200 mt-1">
            {formatCurrency(summary.B.totalSpent)}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-900/30 to-green-800/20 border border-green-800 rounded-lg p-4">
          ...
          <div className="text-sm text-green-200 mt-1">
            {formatCurrency(summary.C.totalSpent)}
          </div>
        </div>
      </div>
        ...
        <BarChart data={chartData}>
          ...
          <Bar dataKey="gasto" fill="#8884d8" name="Gasto Total">
            {chartData.map((entry, index) => {
              const className = entry.name.includes("A") ? "A" : entry.name.includes("B") ? "B" : "C";
      ...
      <div className="mt-4 bg-blue-900/20 border border-blue-800 rounded-lg p-4">
        <p className="text-sm text-blue-300">
          💡 <strong>Análisis Pareto (80-15-5):</strong> Clase A representa ~80% del gasto (pocos proveedores clave), 
          Clase B ~15% (proveedores moderados), y Clase C ~5% (muchos proveedores ocasionales).
        </p>
      </div>
    </div>
  );
