...
export function PurchasesOptimizationPanel({
  startDate,
  endDate,
}: PurchasesOptimizationPanelProps) {
  ...
  const insights = useMemo(() => {
    ...
    purchases.content.forEach(p => {
      ...
      const group = supplierGroups.get(p.supplierId) || {
        count: 0,
        total: 0,
        name: p.supplierName || '',
      }
    ...
    const smallSuppliers = Array.from(supplierGroups.entries()).filter(
      ([_, data]) => data.count <= 2 && data.total < 50000
    ).length
    ...
    const priceAnalysis = new Map<
      string,
      { min: number; max: number; avg: number; count: number; name: string }
    >()
    ...
    const highVariability = Array.from(priceAnalysis.entries()).filter(([_, data]) => {
  ...
  const totalPotentialSaving = insights.reduce(
    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl shadow-lg p-6">
      ...
      <div className="space-y-3">
        {insights.map(insight => {
          ...
          return (
            <div key={insight.id} className={`border rounded-lg p-4 ${typeColors[insight.type]}`}>
