    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-white">
          📊 Pronósticos Detallados por Proveedor
        </h3>
        ...
        <table className="w-full">
          ...
          <tbody>
            {forecasts.map((forecast, index) => {
              ...
              const variation =
                forecast.historicalAverage > 0
                  ? ((forecast.forecastedSpending - forecast.historicalAverage) /
                ...
                >
                  <td className="py-3 px-4">
                    ...
                    <div className="text-xs text-neutral-500">
                      ID: {forecast.supplierId.substring(0, 8)}
                    </div>
                  </td>
                  <td className="py-3 px-4 text-right">
                    <span className="text-neutral-300">
                      {formatCurrency(forecast.historicalAverage)}
                    </span>
                    <div className="text-xs text-neutral-500">por mes</div>
                  </td>
                  <td className="py-3 px-4 text-right">
                    <span className="text-white font-semibold">
                      {formatCurrency(forecast.forecastedSpending)}
                    </span>
                    <div
                      className={`text-xs ${variation >= 0 ? 'text-red-400' : 'text-green-400'}`}
                    >
                      {variation >= 0 ? '+' : ''}
                      {variation.toFixed(1)}%
                    </div>
                  ...
                  <td className="py-3 px-4 text-center">
                    <span
                      className={`inline-flex px-2 py-1 rounded text-xs font-medium border ${trendBadge.style}`}
                    >
                      {trendBadge.label}
                    </span>
                  </td>
