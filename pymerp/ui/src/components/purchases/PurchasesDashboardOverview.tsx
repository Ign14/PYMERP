...
export default function PurchasesDashboardOverview({
  ...
  const stats = useMemo(() => {
    ...
    const pending = purchases.filter(
      p => p.status?.toLowerCase() !== 'received' && p.status?.toLowerCase() !== 'cancelled'
    )
    ...
    const previousReceived = previousPurchases.filter(
    ...
    const countVariation =
      previousCount > 0 ? ((purchases.length - previousCount) / previousCount) * 100 : 0
    const dailyVariation =
      previousDailyAverage > 0
        ? ((dailyAverage - previousDailyAverage) / previousDailyAverage) * 100
        : 0
    const receivedVariation =
      previousReceived > 0 ? ((received.length - previousReceived) / previousReceived) * 100 : 0
    ...
    if (cancelled.length > purchases.length * 0.1)
      alerts.push({ type: 'error', message: 'Alta tasa de cancelación' })
    if (pending.length > received.length)
      alerts.push({ type: 'info', message: 'Muchas órdenes pendientes' })
      ...
      >
        ...
        <div className="kpi-grid" style={{ marginBottom: '1.5rem' }}>
          ...
          <article className="bg-neutral-900 border border-neutral-800 rounded-2xl shadow-lg p-5 stat">
            ...
            <span
              className="muted small text-neutral-400"
          ...
          <article className="bg-neutral-900 border border-neutral-800 rounded-2xl shadow-lg p-5 stat">
            ...
            <div
              style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem', fontSize: '0.875rem' }}
            >
                ...
                <div style={{ fontSize: '0.875rem' }}>
                  {stats.topSuppliers.slice(0, 3).map((supplier, idx) => (
                    <div key={idx} style={{ marginBottom: '0.5rem' }}>
                      ...
                      <div
                        className="bg-neutral-800 rounded-full h-1.5"
