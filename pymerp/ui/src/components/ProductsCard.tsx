  ...
  const handleSaveCriticalChanges = async () => {
    ...
    for (const [productId, value] of entries) {
      try {
        ...
        const productName =
          productsQuery.data?.content?.find(item => item.id === productId)?.name ?? productId
        setCriticalErrors(prev => ({
          ...prev,
    ...
    <div className="card products-card">
      ...
      <div className="inline-actions">
        ...
        <button
          className="btn ghost"
          type="button"
          onClick={() => openQrModal(selectedProduct)}
          disabled={!selectedProduct}
        >
          Ver QR
        </button>
      ...
      {dirty && (
        <p className="muted small notice-dirty">
          Recuerda guardar para aplicar los cambios pendientes.
        </p>
      )}
      {deleteMutation.isError && (
        <p className="error">
          {(deleteMutation.error as Error)?.message ?? 'No se pudo eliminar el producto'}
        </p>
      )}

      {statusMutation.isError && (
        <p className="error">
          {(statusMutation.error as Error)?.message ?? 'No se pudo actualizar el estado'}
        </p>
      )}
                  ...
                  <div className="product-card-body">
                    ...
                    <div className="product-card-stats">
                      <div className="product-stat">
                        ...
                        <span className="product-stat-value mono">
            ...
            <>
              <div className="panel">
                ...
                <form className="form-inline" onSubmit={onSubmitPrice}>
                  <input
                    ...
                    onChange={e =>
                      setPriceForm(prev => ({ ...prev, price: Number(e.target.value) }))
                    }
                  />
                ...
                {priceMutation.isError && (
                  <p className="error">{(priceMutation.error as Error)?.message}</p>
                )}
              ...
              <div className="panel">
                ...
                <button
                  className="btn ghost"
                  type="button"
                  onClick={() => openCriticalStockModal()}
                >
