...
export default function CustomersPage() {
  ...
  const allStatsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const statsPromises = customers.map(c => 
        getCustomerStats(c.id).catch(() => null)
      );
    ...
    <div className="page-section bg-neutral-950">
      ...
      <section className="responsive-grid">
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl shadow-lg p-5">
          ...
          {segmentsQuery.isError && <p className="error text-red-400">{segmentsQuery.error?.message ?? "No se pudieron cargar los segmentos"}</p>}
          {!segmentsQuery.isLoading && !segmentsQuery.isError && (
            <>
              <ul className="segment-list">
                ...
                {segments.length === 0 && <li className="muted text-neutral-400">Sin segmentos registrados</li>}
              </ul>
              {activeSegment && (
